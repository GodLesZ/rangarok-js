  ...
  intersectObject: function (object) {
    ...
    if (object instanceof THREE.Particle) {
      ...
      for (var i = 0, m = object.children.length; i < m; i++) {
		...
		if(!object.material.map.image || !object.material.map.image.width)
			return intersects;
		
		distance = distanceFromIntersection( this.origin, this.direction, object.matrixWorld.getPosition() );
		
		var hitRadius;
		var width = object.material.map.image.width;
		var height = object.material.map.image.height;
		if(object.scale.x == object.scale.y && width == height ) hitRadius = object.scale.x * width / 2;
		else hitRadius = Math.max(object.scale.x * width / 2, object.scale.y * height / 2);
